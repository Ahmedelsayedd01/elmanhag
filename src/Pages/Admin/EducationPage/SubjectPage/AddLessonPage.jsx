...
const AddLessonPage = () => {
  ...
  //   } else {
  //     try {
  //       const response = await axios.post(`http://62.84.185.153/plesk-site-preview/bcknd.elmanhag.com/https/62.84.185.153/admin/lesson/add/${chapterID}`, dataArr, {
  //         headers: {
  ...
  const handleSubmitAdd = async (event) => {
    ...
    try {
      const response = await axios.post(`http://62.84.185.153/plesk-site-preview/bcknd.elmanhag.com/https/62.84.185.153/admin/lesson/add/${chapterID}`, formData, {
        headers: {
          Authorization: `Bearer ${auth.user.token}`,
          'Content-Type': 'multipart/form-data',
        },
