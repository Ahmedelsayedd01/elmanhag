...
const EditBundlesPage = () => {
    ...
    useEffect(() => {
        if (bundleEdit) {
            ...
            setNameAr(bundleEdit.ar_name || '');
            setPrice(bundleEdit.price || '');
            setThumbnail(bundleEdit.thumbnail || '');
            setCoverPhoto(bundleEdit.cover_photo || '');
            setDemoVideo(bundleEdit.demo_video|| '');
            setDescription(bundleEdit.description || '');
            setUrl(bundleEdit.url || '');
            setBundleTags(bundleEdit.tags|| '')
            setBundleActive(bundleEdit.status|| false)
            setExpiredDate(bundleEdit.expired_date|| '')
            setSelectSemester(bundleEdit.semester || 'Select Semester');

            if (bundleEdit.category) {
              setSelectCategory(bundleEdit.category.name);
              setSelectCategoryId(bundleEdit.category.id);
            } else {
