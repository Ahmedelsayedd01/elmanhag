...
const EditAdminPage = () => {
       ...
       useEffect(() => {
              const fetchAdminEdit = async () => {
                     setIsLoading(true);
                     try {
                            const response = await axios.get(
                                   `http://62.84.185.153/plesk-site-preview/bcknd.elmanhag.com/https/62.84.185.153/admin/admins/admin/${adminId}`,
                                   {
                                          headers: {
                            ...
                            if (response.status === 200) {
                                   console.log('response admin edit', response.data);
                                   const adminData = response.data.admins;
                                   // setRoleData(roles);
                                   setAdminName(adminData?.name || adminName)
                                   setAdminPhone(adminData?.phone || adminPhone)
                                   setAdminEmail(adminData?.email || adminEmail)
                                   setAdminPassword(adminPassword)
                                   setRoleId(adminData?.admin_position?.id || roleId)
                                   setAdminStatus(adminData?.status || adminStatus)
                                   setRoleState(adminData?.admin_position?.name || roleState)
                            }
                     } catch (error) {
              ...
              fetchAdminEdit(); // Fetch role initially and whenever roleChanged changes
       ...
       const handleSubmit = async (e) => {
              ...
              try {
                     ...
                     const response = await axios.post(`http://62.84.185.153/plesk-site-preview/bcknd.elmanhag.com/https/62.84.185.153/admin/admins/update/${adminId}`, formData, {
                            headers: {
                                   Authorization: `Bearer ${auth.user.token}`,
