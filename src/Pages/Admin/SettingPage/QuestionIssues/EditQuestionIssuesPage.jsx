...
const EditQuestionIssuesPage = () => {
  const { questionIssuesId } = useParams();
  ...
  useEffect(() => {
    const fetchQuestionIssues = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`https://bdev.elmanhag.shop/admin/Settings/questionIssues/issue/${questionIssuesId}`, {
          headers: {
        ...
        if (response.status === 200) {
          const dataEdit = response.data.question_issue;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`https://bdev.elmanhag.shop/admin/Settings/questionIssues/update/${questionIssuesId}`, payload, {
        headers: {
          Authorization: `Bearer ${auth.user.token}`,
      ...
      if (response.status === 200) {
        handleGoBack();
        auth.toastSuccess("Question Issues Edited Successfully!");
      }
    } catch (error) {
      auth.toastError(`Error: ${error}`);
    } finally {
...
export default EditQuestionIssuesPage