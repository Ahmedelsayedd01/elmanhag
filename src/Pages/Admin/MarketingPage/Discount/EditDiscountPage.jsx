...
import { DiscountEditContext } from '../../../../Layouts/Admin/EditDiscountLayout';

const EditDiscountPage = () => {

  const auth = useAuth();
  const navigate = useNavigate();
  const discountEdit = useContext(DiscountEditContext);
    ...
    if (discountEdit) {
      ...
      if (categoryData !== null) {
        ...
        if (filteredCategoryData) {
          ...
          setSelectCategoryId(filteredCategoryData.id);
        }
        else {
  ...
  const handleSelectValueType = (e) => {
    ...
    setOpenSelectValueType(false);
    console.log('Selected ValueType:', selectedOptionValue);
  };
  ...
  const handleSubmitEdit = async (discountID, event) => {
    ...
    try {
      ...
      selectSubjectId.forEach((subjectId, index) => {
        params.append(`subject_id[${index}]`, subjectId);
      });

      // Append the bundles array
      selectBundleId.forEach((bundleId, index) => {
        params.append(`bundle_id[${index}]`, bundleId);
      });

      // Append other fields
      params.append('category_id', selectCategoryId);
