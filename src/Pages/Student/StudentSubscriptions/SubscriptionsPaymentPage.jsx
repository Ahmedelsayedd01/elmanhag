...
const SubscriptionsPaymentPage = () => {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState('');
  const [code, setCode] = useState('');
  const [newPrice, setNewPrice] = useState(''); // State to hold the new price after applying the promo code
  ...
  const handleNextClick = () => {
    ...
    if (!selectedPaymentMethod) {
      ...
      // Check if newPrice is defined (not null or undefined), even if it's 0
      if (newPrice || newPrice === 0) {
        total = newPrice;
        console.log("New price used:", newPrice);
      } else {
        // Check if plan is an array
        if (Array.isArray(plan)) {
          plan.forEach(singlePlan => {
            // Check if discount_price exists, else use price
            const price = singlePlan.discount_price > 0 ? singlePlan.discount_price : singlePlan.price;
            total += price; // Accumulate the total price
            console.log("Accumulated total:", total);
          });
        } else {
          // Handle case where plan is a single object
          total = plan.discount_price > 0 ? plan.discount_price : plan.price;
          console.log("Single plan total:", total);
        }
      }
  
      // Set the priceToSend to the calculated totalPrice
      const priceToSend = total;
      console.log("Total price to send:", priceToSend);
  
      // Log the details and navigate to the next page
      console.log(plan, selectedPaymentMethod, priceToSend);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // if (Array.isArray(plan)) {
      //   plan.forEach(singlePlan => {
      //     // Calculate price for each plan and accumulate total
      //     const price = singlePlan.price_discount > 0 ? singlePlan.price_discount : singlePlan.price;
      //     totalPrice += price; // Accumulate total price
      //     planIds.push(singlePlan.id); // Collect plan IDs
      //   });
      // } else {
      //   // Handle case where plan is a single object
      //   totalPrice = plan.price_discount > 0 ? plan.price_discount : plan.price;
      //   planIds.push(plan.id);
      // }
      // // Append data to formData
      // formData.append('price', totalPrice);
      // formData.append('code', code);
      // formData.append('id', JSON.stringify(planIds)); // Serialize planIds into a JSON string
      // formData.append('type', 'Subject'); // Set type to 
      ...
      if (planType === 'Subject') {
        ...
        if (Array.isArray(plan)) {
          ...
          // Append plan IDs as a string list (JSON format)
          formData.append('id', JSON.stringify(planIds)); 
        } else {
          // Handle single plan object
          totalPrice = plan.price_discount > 0 ? plan.price_discount : plan.price;
          formData.append('id', JSON.stringify([plan.id])); // Send id as string list
        }
        
  // Append the total price and other necessary fields
  formData.append('price', totalPrice);
  formData.append('type', 'Subject'); // Plan type is Subject

  } else {

      totalPrice = plan.price_discount > 0 ? plan.price_discount : plan.price;
      formData.append('id', plan.id); // Send id as integer
      formData.append('price', totalPrice);
      formData.append('type', planType); // Use the provided plan type
  }

    // Append other fields like 'code' as needed
    formData.append('code', code);
  ...
  return (
    <div className="p-6 min-h-screen">
      <form onSubmit={handleSubmit} className="w-full flex flex-col">   
        ...
        <div className="flex flex-col lg:flex-row-reverse w-full gap-8">
          ...
          {/* <div className="flex flex-col justify-center bg-white p-4 rounded-lg shadow-lg text-center w-full sm:w-2/3 md:w-1/2 lg:w-1/3 mx-auto transform transition-transform hover:scale-105">
            ...
            <div className='flex flex-col gap-4 w-full lg:w-2/4'>
              {Array.isArray(plan) ? (
                plan.map((singlePlan) => (
                  <div key={singlePlan.id} className="flex flex-col justify-center bg-white p-4 rounded-lg shadow-lg text-center w-full lg:w-2/3 mx-auto transform transition-transform hover:scale-105">
                  <div className="flex justify-center">
                  <div className="mb-4 flex flex-col items-center">
                    <img  
                      src={singlePlan.cover_photo_url} 
                      alt={singlePlan.name} 
                      className="max-w-full h-auto rounded-lg" 
                    />
                    <h2 className="text-xl sm:text-2xl font-semibold text-mainColor mt-2">{singlePlan.name}</h2>
                    <p className="text-gray-700 mb-4 mt-2">
                      {newPrice ? (
                        <span className="text-3xl text-mainColor font-semibold">{newPrice} جنيه</span>
                      ) : (
                        <>
                          {singlePlan.price_discount > 0 ? (
                            <>
                              <span className="text-3xl text-mainColor font-semibold">{singlePlan.price_discount} جنيه</span> بدلا من
                              <span className="line-through text-xl text-gray-600">{singlePlan.price} جنيه</span>
                            </>
                          ) : (
                            <span className="text-3xl text-mainColor font-semibold">{singlePlan.price} جنيه</span>
                          )}
                        </>
                      )}
                    </p>    
              ...
              : (
                <div className="flex flex-col justify-center bg-white p-4 rounded-lg shadow-lg text-center  w-full lg:w-2/3 mx-auto transform transition-transform hover:scale-105">
                  <div className="flex justify-center">
          ...
          <div className="flex flex-col gap-4 w-full lg:w-3/4">
            ...
            {planPayment.map((method) => (
              ...
              >
                <div className={`w-6 h-6 rounded-full border-2 ${selectedPaymentMethod.title === method.title ? 'border-red-500 bg-mainColor' : 'border-gray-300'} mr-3`} />
                <img src={method.thumbnail_link} alt={method.title} className={`${method.title === "Vodafone cash" ? '' : 'h-14'}`} />
                <h3 className="text-lg font-semibold">{method.title}</h3>
              </div>
            ))}
        ...
        <div className="flex items-center justify-center w-72 mt-10 rounded-2xl">
              <Button handleClick={handleNextClick} Text={"التالي"} BgColor="bg-mainColor" Color="text-white" Width='full' Size='text-2xl' px='px-28' rounded='rounded-2xl' stateLoding={isLoading} />
        </div>
    </div>
  );
};
